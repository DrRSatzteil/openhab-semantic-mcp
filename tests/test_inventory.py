...
class TestInventoryIndexing:
    ...
    def setup_method(self):
        ...
        self.living_room = Location(name="Indoor_Room_LivingRoom", label="Living Room")
        self.kitchen = Location(name="Indoor_Room_Kitchen", label="Kitchen")
        self.garden = Location(name="Outdoor_Garden", label="Garden")
    ...
    def test_location_type_hierarchy(self):
        ...
        assert len(indoor_room_items) == 3  # Living room + kitchen items (both fall under Indoor_Room)
    ...
    def test_combined_filters(self):
        ...
        # Equipment + Property
        lighting_light = self.inventory.get(
            equipment="Lighting",
            item_property="Light"
        )
    ...
    def test_state_filtering(self):
        ...
        from openhab_semantic_mcp.inventory import ExactStateFilter
        
        on_items = self.inventory.get(
            state=ExactStateFilter(states=["ON"])
        )
        assert len(on_items) == 1
        assert "LivingRoom_Light" in on_items
        
        off_items = self.inventory.get(
            state=ExactStateFilter(states=["OFF"])
        )
